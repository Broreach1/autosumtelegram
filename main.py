...
def update_total(chat_id, currency, amount):
    now = datetime.now()
    shift, biz = get_shift_and_business_date(now)
    ...
    cur.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    """, (chat_id, now.strftime("%Y-%m-%d %H:%M:%S"), biz, shift, currency, amount))

    cur.execute("""
        SELECT total, invoices FROM totals
        WHERE chat_id=? AND date=? AND shift=? AND currency=?
    """, (chat_id, biz, shift, currency))
    ...
    if row:
        cur.execute("""
            ...
            WHERE chat_id=? AND date=? AND shift=? AND currency=?
        """, (row[0] + amount, row[1] + 1, chat_id, biz, shift, currency))
    else:
        cur.execute("""
            INSERT INTO totals VALUES (?, ?, ?, ?, ?, ?)
        """, (chat_id, biz, shift, currency, amount, 1))
